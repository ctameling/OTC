rm(list = ls())
######################################################################################################
####### Script for Supplemenatry Figure 3 ############################################################
...
# get data path
data_path <- "../data/simulated_data/SupplFig2_dense_structures"
...
for (i in data_sets){
  ...
  # compute tplans
  tplans <- OTC::calculate_tplans(data_path = data_path_i, picsA = picsA, picsB = picsB, output_path = output_path, output_name = paste("dense_structure", i, sep="_"))
...
# evaluate OTC
data_list <- paste("Tplans_dense_structure_", data_sets, ".RData", sep="")
dim <- c(128)
pxsize <- 15
otc_curves <- OTC::evaluate_tplans(data_path = output_path, data_list=data_list, pxsize=pxsize, dim=dim, output_path=output_path, output_name="dense_structure")

# plot otc curves
OTC::plot_otc_curves(otc_curves = otc_curves, output_path = output_path, output_name = "dense_structures_suppl_fig2")
